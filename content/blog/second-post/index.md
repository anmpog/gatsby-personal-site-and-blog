...
At the most general level, looping is a technique for getting stuff done. We use looping in real life whenever we are accomplishing _any_ repetitive task. This begs the question: what is a repetitive task? I think a really great example of a repetitive task would be filling and mailing envelopes. This is a task that we can go about accomplishing using a concrete set of steps that don't vary much from envelope to envelope.
...
In real life, we are likely a little more relaxed about the order in which we complete the steps above. We might stuff a few envelopes at a time before addressing them, stuff a few more, then affix the stamps on the envelopes at the very end of the process. There are many ways we could (and probably would) go about doing this if we were doing this "for real". Nonetheless, this is what looping looks like outside of coding.
...
1.) <ExternalLink to={'https://developer.mozilla.org/en-US/docs/Learn/JavaScript/Building_blocks/Looping_code'}>Looping code</ExternalLink> from Mozilla Developer Network - the gold standard for information on web development.

2.) Though <ExternalLink to={'https://www.dataquest.io/blog/python-for-loop-tutorial/'}>this example</ExternalLink> is in Python, it still discusses looping in a general way and the concepts discussed are applicable to any language.

3.) <ExternalLink to={'https://www.youtube.com/watch?v=eSYeHlwDCNA'}>This video</ExternalLink> from CodeCademy does basically what the article above was aiming for, but in video form.
